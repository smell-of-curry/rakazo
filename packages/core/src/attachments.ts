...
export function validateBotAvatarMimeType(mimeType: string): void {
  if (!(BOT_AVATAR_MIME_TYPES as readonly string[]).includes(mimeType)) {
    throw new AttachmentValidationError(`Unsupported avatar type: ${mimeType}`);
  }
}

export function sniffBotAvatarMimeType(
  bytes: Uint8Array,
): (typeof BOT_AVATAR_MIME_TYPES)[number] | null {
...
export function decodeBotAvatarBase64(contentBase64: string): Uint8Array {
  const bytes = decodeAttachmentBase64(contentBase64);
  if (bytes.byteLength > BOT_AVATAR_MAX_BYTES) {
    throw new AttachmentValidationError("Avatar exceeds the 2 MiB limit");
  }
  return bytes;
}
