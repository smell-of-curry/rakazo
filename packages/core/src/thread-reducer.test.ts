...
describe("mobile-shaped snapshots and takeover lock", () => {
  it("clears leftover progress when takeover is requested", () => {
    const run = threadRun("run-1");
    const initial: ThreadSnapshot = {
      ...snapshot([
        {
    ...
    const waiting = reduceThreadSnapshot(
      initial,
      event({ type: "computer.takeover.requested", seq: 10, runId: run.id }),
    );

    expect(waiting?.run?.status).toBe("waiting_takeover");
    expect(waiting?.messages.some((item) => item.id.startsWith("progress:"))).toBe(false);
  });

  it("inserts a peer takeover run and clears snapshot busyBotName", () => {
    const userRun = threadRun("run-user");
    const initial: ThreadSnapshot = {
      ...snapshot([
        {
          ...message("progress:run-peer", [{ kind: "progress", text: "working…" }]),
          runId: "run-peer",
        },
      ]),
      run: userRun,
      activeRuns: [userRun],
      computer: computer({ state: "running", busyBotName: "Peer" }),
    };
    ...
    expect(waiting?.activeRuns?.map((item) => ({ id: item.id, status: item.status }))).toEqual([
      { id: "run-user", status: "running" },
      { id: "run-peer", status: "waiting_takeover" },
    ]);
    expect(waiting?.messages.some((item) => item.id.startsWith("progress:"))).toBe(false);
  ...
  it("applies a batch of events in order", () => {
    const next = applyThreadEvents(snapshot([]), [
      event({ type: "run.started", seq: 4, runId: "run-1", payload: { trigger: "user" } }),
      event({ type: "thread.progress", seq: 5, runId: "run-1", payload: { text: "Hi" } }),
      event({ type: "run.waiting_input", seq: 6, runId: "run-1" }),
    ]);
