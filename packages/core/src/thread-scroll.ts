export const THREAD_NEAR_END_PX = 80;

export function isNearThreadEnd(distanceFromEnd: number): boolean {
  return distanceFromEnd < THREAD_NEAR_END_PX;
}

export function threadMovedDown(previousScrollTop: number | null, scrollTop: number): boolean {
...
export class ThreadScrollBehavior {
  ...
  onUserScroll(distanceFromEnd: number): ThreadScrollState {
    const detached = distanceFromEnd > THREAD_NEAR_END_PX;
    this.currentState = {
      detached,
