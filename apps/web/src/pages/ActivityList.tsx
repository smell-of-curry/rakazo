...
export function ActivityList({ onOpenRun }: ActivityListProps) {
  ...
  if (loading) {
    return (
      <div className="px-2.5 py-2 text-body text-muted-foreground/80">
  ...
  return (
    <div className="mb-2 border-b border-border pb-2">
      {activeRuns.length > 0 ? (
        <section>
          <div className="px-2.5 pb-1 pt-1 text-caption font-medium text-muted-foreground/80">
            <Trans>Now</Trans>
          </div>
          {activeRuns.map((run) => (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <span className="shrink-0 text-small text-muted-foreground/80">
        ...
        <div className="mt-0.5 flex items-baseline gap-2">
          ...
          <span className={`ms-auto shrink-0 text-small ${tone}`}>{label}</span>
        </div>
      </div>
    </button>
  );
