...
test("takeover, routine, plugins, and export are reachable", async ({ page }, testInfo) => {
  ...
  await expect(overlay.getByRole("tab", { name: "Installed" })).toBeVisible();
  const featured = overlay.getByTestId("featured-connectors");
  ...
  await overlay.getByRole("tab", { name: "Installed" }).click();
  const installedLinear = overlay
    .getByText("Linear", { exact: true })
    .locator("xpath=ancestor::*[.//button][1]");
  await installedLinear.getByRole("button", { name: "Remove", exact: true }).click();
  await overlay.getByRole("tab", { name: "Marketplace" }).click();
  ...
  await addMcp.getByLabel("Server URL").fill("https://mcp.example.test/mcp");
  await addMcp.getByRole("button", { name: "Add server", exact: true }).click();
  await expect(overlay.getByText("Browser MCP", { exact: true })).toBeVisible();
  ...
  await expect(settings.getByRole("button", { name: "Delete bot" })).toBeVisible();
...
test("sign-in, spawn, and stop work in the shell", async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId("composer-bar").getByRole("button", { name: "Send" })).toHaveCount(
    0,
  );
